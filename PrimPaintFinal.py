...
class Paint(object):
    ...
    def openFile(self):
        global opened_image
        image_formats=[('All Image Files', "*.png *.jpeg *.jpg"),("PNG Files", "*.png"), ("JPEG Files", "*.jpeg"),("JPG Files", "*.jpg"),("All Files", "*.*")]
        filename = filedialog.askopenfilename(initialdir="/", title="Select a File", filetypes=(image_formats))
        print (filename)
        if not filename.lower().endswith(('.png', '.jpg', '.jpeg','.PNG', '.JPG', '.JPEG',)):
            messagebox.showwarning("File Open", "This File Type is not Supported")
        else:
